...
public class GPAViewControl {
    ...
    void displayOverallGPA() {
        gpa.calcOverallGPA(grades, s, course);
        overallGPAField.setText(gpa.toString());
    }
    
    @FXML
    void displaySemesterGPAAction(ActionEvent event) {
        int semester = semesterChoiceBox.getSelectionModel().getSelectedItem();
        gpa.calcSemesterGPA(grades, s, course, semester);        
        
        semesterGPAField.setText(Double.toString(gpa.getSemesterGPA()));//this line should be removed once it actually works, just makes sure the things are speaking to each other
